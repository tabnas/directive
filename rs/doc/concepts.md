...
`Rule::with_shared_node(name, parent.node.clone())` — it shares the
...
| **Assigning a node** | `rule.node = value`. | `set_node(rule, value)` — a pushed rule shares its parent's node cell, so an assignment must install a fresh one. |
