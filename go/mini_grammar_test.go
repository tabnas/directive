...
func registerMiniGrammar(j *tabnas.Tabnas) {
	...
	j.Rule("val", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
		rs.BO = []tabnas.StateAction{func(r *tabnas.Rule, _ *tabnas.Context) {
	...
	j.Rule("map", func(rs *tabnas.RuleSpec, _ *tabnas.Parser) {
		...
		rs.Open = []*tabnas.AltSpec{
			{S: [][]tabnas.Tin{{tabnas.TinOB}, {tabnas.TinCB}}, B: 1, N: map[string]int{"pk": 0}},
			{S: [][]tabnas.Tin{{tabnas.TinOB}}, P: "pair", N: map[string]int{"pk": 0}},
		}
		rs.Close = []*tabnas.AltSpec{
			{S: [][]tabnas.Tin{{tabnas.TinCB}}},
		}
	})
