...
//
// libtabnasdirective is the directive format parser as a C shared library, one of
...
const (
	templateVersion = "v5"
	libName         = "libtabnasdirective"
	formatName      = "directive"
	valueOut        = true
...
// newParser builds one engine with the directive grammar installed
...
func newParser(opts string) (parseFn, error) {
	off := false; tn := host.Make(host.Options{Color: &host.ColorOptions{Active: &off}}); gs, err := host.GrammarSpecFromJSON([]byte(opts)); if err != nil { return nil, &host.TabnasError{Code: "grammar", Detail: "unreadable spec: " + err.Error()} }; if err := tn.Grammar(gs); err != nil { return nil, err }; start := tn.Config().RuleStart; if start == "" { start = "val" }; if tn.RSM()[start] == nil { return nil, &host.TabnasError{Code: "grammar", Detail: "spec installs no start rule " + start + ", so no input could be validated against it"} }; upper := func(r *host.Rule, _ *host.Context) { if s, ok := r.Child.Node.(string); ok { r.Node = strings.ToUpper(s) } else { r.Node = r.Child.Node } }; adder := func(r *host.Rule, _ *host.Context) { sum := 0.0; if l, ok := r.Child.Node.([]any); ok { for _, v := range l { if n, ok := v.(float64); ok { sum += n } } }; r.Node = sum }; if _, err := plug.Apply(tn, plug.DirectiveOptions{Name: "upper", Open: "@", Action: upper}); err != nil { return nil, err }; if _, err := plug.Apply(tn, plug.DirectiveOptions{Name: "adder", Open: "add<", Close: ">", Action: adder}); err != nil { return nil, err }; value, ok := host.BUILTIN_REFS["@value$"].(host.AltAction); if !ok { panic("directive: the engine has no @value$ builtin") }; closeTin, ok := tn.Config().FixedTokens[">"]; if !ok { panic("directive: the adder close token is not registered") }; patched := 0; tn.Rule("val", func(rs *host.RuleSpec, _ *host.Parser) { for _, alt := range rs.CloseAlts() { if alt.A == nil && alt.B == 1 && strings.Contains(","+alt.G+",", ",directive,") && len(alt.S) == 1 && len(alt.S[0]) == 1 && alt.S[0][0] == closeTin { alt.A = value; patched++ } } }); if patched != 1 { panic("directive: expected exactly one close alternate inserted into val by the adder directive") }; var cyclic func(any, map[any]bool) bool; cyclic = func(n any, on map[any]bool) bool { var key any; var kids []any; switch t := n.(type) { case *host.ListRef: if t == nil { return false }; key, kids = t, t.Val; case []any: if len(t) == 0 { return false }; key, kids = &t[0], t; case *host.OrderedMap: if t == nil { return false }; key = t; for _, k := range t.Keys { kids = append(kids, t.Vals[k]) }; default: return false }; if on[key] { return true }; on[key] = true; defer delete(on, key); for _, c := range kids { if cyclic(c, on) { return true } }; return false }; return func(src string) (any, error) { v, err := tn.Parse(src); if err != nil { return nil, err }; if cyclic(v, map[any]bool{}) { panic("directive: the parse produced a value that contains itself") }; return v, nil }, nil
}
...
// parseWith answers whether src is in the directive language — and,
