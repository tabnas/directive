...
func newParser(opts string) (parseFn, error) {
	...
	upper := func(r *host.Rule, _ *host.Context) {
		if s, ok := r.Child.Node.(string); ok {
			r.Node = strings.ToUpper(s)
		} else {
			r.Node = r.Child.Node
		}
	}
	adder := func(r *host.Rule, _ *host.Context) {
		sum := 0.0
		if l, ok := r.Child.Node.([]any); ok {
			for _, v := range l {
	...
	if _, err := plug.Apply(tn, plug.DirectiveOptions{Name: "upper", Open: "@", Action: upper}); err != nil {
		return nil, err
	}
	if _, err := plug.Apply(tn, plug.DirectiveOptions{Name: "adder", Open: "add<", Close: ">", Action: adder}); err != nil {
		return nil, err
	}
	value, ok := host.BUILTIN_REFS["@value$"].(host.AltAction)
	if !ok {
		panic("directive: the engine has no @value$ builtin")
	}
	closeTin, ok := tn.Config().FixedTokens[">"]
	if !ok {
		panic("directive: the adder close token is not registered")
	}
	patched := 0
	tn.Rule("val", func(rs *host.RuleSpec, _ *host.Parser) {
		for _, alt := range rs.CloseAlts() {
			if alt.A == nil && alt.B == 1 && strings.Contains(","+alt.G+",", ",directive,") && len(alt.S) == 1 && len(alt.S[0]) == 1 && alt.S[0][0] == closeTin {
				alt.A = value
				patched++
			}
		}
	...
	if patched != 1 {
		panic("directive: expected exactly one close alternate inserted into val by the adder directive")
	}
	...
	return func(src string) (any, error) {
		...
		if cyclic(v, map[any]bool{}) {
			panic("directive: the parse produced a value that contains itself")
		}
		return v, nil
	}, nil
}
